/*
 * arch/arm/mach-tegra/board-endeavortd.c
 *
...
static struct platform_device endeavortd_rfkill = {
	.name = "endeavortd_rfkill",
	.id = -1,
};
...
int plat_kim_suspend_td(struct platform_device *pdev, pm_message_t state)
{
...
int plat_kim_resume_td(struct platform_device *pdev)
{
	retry_suspend = 0;
        return 0;
}

struct ti_st_plat_data wilink_pdata_td = {
                .nshutdown_gpio = 160,
                .dev_name = "/dev/ttyHS2",
                .flow_cntrl = 1,
                .baud_rate = 3000000,
                /*.baud_rate = 115200,*/
                .suspend = plat_kim_suspend_td,
                .resume = plat_kim_resume_td,
...
static struct platform_device wl128x_device = {
        .name           = "kim",
        .id             = -1,
        .dev.platform_data = &wilink_pdata_td,
};
...
static __initdata struct tegra_clk_init_table endeavortd_clk_init_table[] = {
...
static struct tegra_i2c_platform_data endeavortd_i2c1_platform_data = {
	.adapter_nr	= 0,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data endeavortd_i2c2_platform_data = {
	.adapter_nr	= 1,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data endeavortd_i2c3_platform_data = {
	.adapter_nr	= 2,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data endeavortd_i2c4_platform_data = {
	.adapter_nr	= 3,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data endeavortd_i2c5_platform_data = {
	.adapter_nr	= 4,
	.bus_count	= 1,
...
static struct htc_asoc_platform_data endeavortd_audio_pdata;

static struct platform_device endeavortd_audio_device = {
	.name	= "tegra-snd-aic3008",
	.id	= 0,
	.dev	= {
		.platform_data  = &endeavortd_audio_pdata,
	},
};

#if (defined(CONFIG_SND_AMP_TFA9887))
static struct tfa9887_platform_data endeavortd_speaker_amp_data = {
...
static struct i2c_board_info i2c_speaker_amp_devices[] = {
	{
		...
		.platform_data = &endeavortd_speaker_amp_data,
		.irq = -1,
	},
};
#endif
...
static struct spi_board_info endeavortd_spi_board_info_rawchip[] __initdata = {
       {
...
EXPORT_SYMBOL_GPL(endeavortd_spi_board_info_rawchip);
...
static void endeavortd_i2c_init(void)
{
	tegra_i2c_device1.dev.platform_data = &endeavortd_i2c1_platform_data;
	tegra_i2c_device2.dev.platform_data = &endeavortd_i2c2_platform_data;
	tegra_i2c_device3.dev.platform_data = &endeavortd_i2c3_platform_data;
	tegra_i2c_device4.dev.platform_data = &endeavortd_i2c4_platform_data;
	tegra_i2c_device5.dev.platform_data = &endeavortd_i2c5_platform_data;
...
static struct platform_device *endeavortd_uart_devices[] __initdata = {
	&tegra_uarta_device,
	&tegra_uartb_device,
	&tegra_uartc_device,
	&tegra_uartd_device,
...
static struct tegra_uart_platform_data endeavortd_uart_pdata;
static struct tegra_uart_platform_data endeavortd_loopback_uart_pdata;

#ifdef CONFIG_SERIAL_TEGRA_BRCM
static struct tegra_uart_platform_data endeavortd_brcm_uart_pdata;
#endif
...
static void __init uart_debug_init(void)
{
	...
	endeavortd_uart_devices[0] = &debug_uarta_device;
...
static void __init endeavortd_uart_init(void)
{
	int i;
	struct clk *c;
	...
	endeavortd_uart_pdata.parent_clk_list = uart_parent_clk;
	endeavortd_uart_pdata.parent_clk_count = ARRAY_SIZE(uart_parent_clk);

/* GPS use UARTE
	endeavortd_loopback_uart_pdata.parent_clk_list = uart_parent_clk;
	endeavortd_loopback_uart_pdata.parent_clk_count =
						ARRAY_SIZE(uart_parent_clk);
	endeavortd_loopback_uart_pdata.is_loopback = true;
*/

	tegra_uarta_device.dev.platform_data = &endeavortd_uart_pdata;
	tegra_uartb_device.dev.platform_data = &endeavortd_uart_pdata;
	tegra_uartc_device.dev.platform_data = &endeavortd_uart_pdata;
	tegra_uartd_device.dev.platform_data = &endeavortd_uart_pdata;
	tegra_uarte_device.dev.platform_data = &endeavortd_uart_pdata; // Used for BRCM GPS.

#ifdef CONFIG_BT_CTS_WAKEUP
	int board_id = htc_get_pcbid_info();

	endeavor_bt_uart_pdata = endeavortd_uart_pdata;
	if ((machine_is_endeavortd() && (board_id >= PROJECT_PHASE_XC)))
	{
...
#ifdef CONFIG_SERIAL_TEGRA_BRCM
	endeavortd_brcm_uart_pdata = endeavortd_uart_pdata;
	endeavortd_brcm_uart_pdata.bt_wakeup_pin_supported = 1;
	endeavortd_brcm_uart_pdata.bt_wakeup_pin = ENDEAVORTD_GPIO_BT_WAKE;
	endeavortd_brcm_uart_pdata.host_wakeup_pin = ENDEAVORTD_GPIO_BT_HOST_WAKE;
	tegra_uartc_device.dev.platform_data = &endeavortd_brcm_uart_pdata;
	tegra_uartc_device.name = "tegra_uart_brcm"; /* for brcm */
#endif

/* GPS uses UARTE
	// UARTE is used for loopback test purpose
	tegra_uarte_device.dev.platform_data = &endeavortd_loopback_uart_pdata;
*/
	...
	platform_add_devices(endeavortd_uart_devices,
				ARRAY_SIZE(endeavortd_uart_devices));
}
...
/*Add SC880x SPI controller support ++*/
static struct serial_sc8800g_platform_data serial_sc8800g_pdata = {
		.gpio[AP_RTS] = {
			.name = "sc8800g_hs_ap_rts",
			.pinnum = SPRD_SC800x_GPIO_AP_RTS,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.offtype = OUTPUT_PIN,
		},
		.gpio[AP_RDY] = {
			.name = "sc8800g_hs_ap_rdy",
			.pinnum = SPRD_SC800x_GPIO_AP_RDY,
			.type = OUTPUT_PIN,
			.init_output = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[AP_TO_MDM1] = {
			.name = "sc8800g_ap_to_mdm1",
			.pinnum = SPRD_SC800x_GPIO_AP_TO_MDM1,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[AP_TO_MDM2] = {
			.name = "sc8800g_ap_to_mdm2",
			.pinnum = SPRD_SC800x_GPIO_AP_TO_MDM2,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[AP_SPI_RTY] = {
			.name = "sc8800g_ap_spy_rty",
			.pinnum = SPRD_SC800x_GPIO_AP_SPI_RTY,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[AP_ALIVE] = {
			.name = "sc8800g_ap_alive",
			.pinnum = SPRD_SC800x_GPIO_AP_ALIVE,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[AP_RESEND] = {
			.name = "sc8800g_ap_resend",
			.pinnum = SPRD_SC800x_GPIO_AP_RESEND,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_RTS] = {
			.name = "sc8800g_hs_mdm_rts",
			.pinnum = SPRD_SC800x_GPIO_MDM_RTS,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_RDY] = {
			.name = "sc8800g_hs_mdm_rdy",
			.pinnum = SPRD_SC800x_GPIO_MDM_RDY,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_TO_AP1] = {
			.name = "sc8800g_mdm_to_ap1",
			.pinnum = SPRD_SC800x_GPIO_MDM_TO_AP1,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_TO_AP2] = {
			.name = "sc8800g_mdm_to_ap2",
			.pinnum = SPRD_SC800x_GPIO_MDM_TO_AP2,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_RESET] = {
			.name = "sc8800g_mdm_reset",
			.pinnum = SPRD_SC800x_GPIO_MDM_RESET,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[MDM_RESEND] = {
			.name = "sc8800g_mdm_resend",
			.pinnum = SPRD_SC800x_GPIO_MDM_RESEND,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_SPI_RTY] = {
			.name = "sc8800g_mdm_spi_rty",
			.pinnum = SPRD_SC800x_GPIO_MDM_SPI_RTY,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.disabled = 1,
			.offtype = OUTPUT_PIN,
		...
		.gpio[MDM_ALIVE] = {
			.name = "sc8800g_mdm_alive",
			.pinnum = SPRD_SC800x_GPIO_MDM_ALIVE,
			.type = INPUT_PIN,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_POWER] = {
			.name = "sc8800g_mdm_power",
			.pinnum = SPRD_SC800x_GPIO_MDM_POWER,
			.type = OUTPUT_PIN,
			.init_output = 0,
			.offtype = OUTPUT_PIN,
		},
		.gpio[MDM_NBOOT] = {
			.name = "sc8800g_mdm_nboot",
			.pinnum = SPRD_SC800x_GPIO_MDM_NBOOT,
			.type = OUTPUT_PIN,
			.init_output = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[SPI_CLK] = {
			.name = "sc8800g_spi_clk",
			.pinnum = SPRD_SC800x_GPIO_SPI_CLK,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[SPI_DO] = {
			.name = "sc8800g_spi_do",
			.pinnum = SPRD_SC800x_GPIO_SPI_DO,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[SPI_CS] = {
			.name = "sc8800g_spi_cs",
			.pinnum = SPRD_SC800x_GPIO_SPI_CS,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[SPI_DI] = {
			.name = "sc8800g_spi_di",
			.pinnum = SPRD_SC800x_GPIO_SPI_DI,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[UART4_RXD] = {
			.name = "sc8800g_uart4_rxd",
			.pinnum = SPRD_SC800x_GPIO_UART4_RXD,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[UART4_CTS] = {
			.name = "sc8800g_uart4_cts",
			.pinnum = SPRD_SC800x_GPIO_UART4_CTS,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[UART4_TXD] = {
			.name = "sc8800g_uart4_txd",
			.pinnum = SPRD_SC800x_GPIO_UART4_TXD,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[UART4_RTS] = {
			.name = "sc8800g_uart4_rts",
			.pinnum = SPRD_SC800x_GPIO_UART4_RTS,
			.type = INPUT_PIN,
			.is_spi = 1,
			.offtype = OUTPUT_PIN,
		},
		.gpio[SIM_DOOR] = {
			.name = "sc8800g_sim_door",
			.pinnum = SPRD_SC800x_GPIO_SIM_DOOR,
			.type = INPUT_PIN,
			.offtype = INPUT_PIN,
		},
		.mdm_rdy_gpio = SPRD_SC800x_GPIO_MDM_RDY,
		.mdm_rts_gpio = SPRD_SC800x_GPIO_MDM_RTS,
		.ap_rdy_gpio = SPRD_SC800x_GPIO_AP_RDY,
		.ap_rts_gpio = SPRD_SC800x_GPIO_AP_RTS,
		.mdm_to_ap1_gpio = SPRD_SC800x_GPIO_MDM_TO_AP1,
		.mdm_alive_gpio = SPRD_SC800x_GPIO_MDM_ALIVE,
		.sim_door_gpio = SPRD_SC800x_GPIO_SIM_DOOR,
};
...
static struct spi_board_info sc8800g_spi_board_info[] __initdata = {
        {
                ...
                .bus_num        = 0,
                .chip_select    = 0,
                .max_speed_hz   = 12750000,
                .platform_data  = &serial_sc8800g_pdata,
                .controller_data        = NULL,
        }
};

struct spi_clk_parent endeavor_spi_parent_clk[] = {
       [0] = {.name = "pll_p"},
#ifndef CONFIG_TEGRA_PLLM_RESTRICTED
       [1] = {.name = "pll_m"},
       [2] = {.name = "clk_m"},
...
static struct tegra_spi_platform_data endeavortd_spi_pdata = {
	.is_dma_based		= true,
	.max_dma_buffer		= (16 * 1024),
	.is_clkon_always	= false,
	.max_rate		= 100000000,
...
static void __init sc8800g_spi_gpio_init(void)
{
	int i = 0;
	struct clk *c;
	struct serial_sc8800g_platform_data* pdata = (struct serial_sc8800g_platform_data*)(sc8800g_spi_board_info->platform_data);

	for (i = 0; i < ARRAY_SIZE(endeavor_spi_parent_clk); ++i) {
		c = tegra_get_clock_by_name(endeavor_spi_parent_clk[i].name);
		if (IS_ERR_OR_NULL(c)) {
			pr_err("[%s] Not able to get the clock for %s\n",
				__func__, endeavor_spi_parent_clk[i].name);
			continue;
		}
		endeavor_spi_parent_clk[i].parent_clk = c;
		endeavor_spi_parent_clk[i].fixed_clk_rate = clk_get_rate(c);
		pr_info("[%s]: fixed_clk_rate=%lu\n", __func__, endeavor_spi_parent_clk[i].fixed_clk_rate);
	}
	endeavor_spi_pdata.parent_clk_list = endeavor_spi_parent_clk;
	endeavor_spi_pdata.parent_clk_count = ARRAY_SIZE(endeavor_spi_parent_clk);

	tegra_spi_device1.dev.platform_data = &endeavor_spi_pdata;

	pr_info("[%s]: get_radio_flag=0x%x\n", __func__, get_radio_flag());

	if ( (get_radio_flag() & 0x400) || (get_radio_flag() & 0x800) ) {
		pdata->gpio[AP_TO_MDM2].init_output = 1;
		pdata->gpio[AP_TO_MDM2].disabled = 0;
		pr_info("[%s]: pdata->gpio[AP_TO_MDM2].init_output=%d\n", __func__, pdata->gpio[AP_TO_MDM2].init_output);
	}

	if ( get_radio_flag() & 0x400 ) {
		wake_lock_init(&g_sc8800g_wakelock_debug, WAKE_LOCK_SUSPEND, "sc8800g_wakelock_debug");
		if (!wake_lock_active(&g_sc8800g_wakelock_debug)) {
			wake_lock(&g_sc8800g_wakelock_debug);
			pr_info("%s: wake_lock [%s]\n",
				__func__, g_sc8800g_wakelock_debug.name);
		} else {
			pr_err("%s: wake_lock [%s] already lock\n",
				__func__, g_sc8800g_wakelock_debug.name);
		}
	}

	if ( get_radio_flag() & 0x80000 ) {
		sprintf( sc8800g_spi_board_info->modalias, "%s", "sc8800g");
		pr_info("[%s]: sc8800g_spi_board_info->modalias=[%s]\n", __func__, sc8800g_spi_board_info->modalias);
	}

	if ( get_radio_flag() & 0x800000 ) {
		pdata->enable_spi_byte_counter = 1;
		pr_info("[%s]: pdata->enable_spi_byte_counter=[1]\n", __func__);
	}

	if ( get_radio_flag() & 0x200 ) {
		pdata->enable_usb_status_to_mdm = 1;
		pr_info("[%s]: pdata->enable_usb_status_to_mdm=[1]\n", __func__);
	}
...
static void __init endeavortd_spi_init(void)
{
	int i;
	struct clk *c;
	/* for enable SP880xG Modem*/
	sc8800g_spi_gpio_init();
	...
	endeavortd_spi_pdata.parent_clk_list = spi_parent_clk;
	endeavortd_spi_pdata.parent_clk_count = ARRAY_SIZE(spi_parent_clk);

	spi_register_board_info(spi_board_info_audio, ARRAY_SIZE(spi_board_info_audio));
	spi_register_board_info(endeavortd_spi_board_info_rawchip, ARRAY_SIZE(endeavortd_spi_board_info_rawchip));
	platform_device_register(&tegra_spi_device2);
	tegra_spi_device4.dev.platform_data = &endeavortd_spi_pdata;
...
static struct platform_device *endeavortd_devices[] __initdata = {
	&tegra_pmu_device,
	&tegra_rtc_device,
	&tegra_udc_device,
...
static int __init endeavortd_touch_init(void)
{
	int retval = 0;
	struct board_info BoardInfo;
	pr_info("[TP]Enter endeavortd_touch_init\n");
...
static int endeavortd_usb_hsic_postsupend(void)
{
	pr_debug("%s\n", __func__);
...
static int endeavortd_usb_hsic_preresume(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static int endeavortd_usb_hsic_phy_ready(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static int endeavortd_usb_hsic_phy_off(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static struct tegra_usb_phy_platform_ops hsic_xmm_plat_ops = {
	.post_suspend = endeavortd_usb_hsic_postsupend,
	.pre_resume = endeavortd_usb_hsic_preresume,
	.port_power = endeavortd_usb_hsic_phy_ready,
	.post_phy_off = endeavortd_usb_hsic_phy_off,
};
...
static void endeavortd_usb_init(void)
{
...
static void endeavortd_cable_detect_init(void)
{
	platform_device_register(&cable_detect_device);
}


static struct platform_device *endeavortd_audio_devices[] __initdata = {
	&tegra_ahub_device,
	&tegra_dam_device0,
	&tegra_dam_device1,
	&tegra_dam_device2,
	...
	&spdif_dit_device,
	&tegra_pcm_device,
	&endeavortd_audio_device,
};

static void endeavortd_audio_init(void)
{
	endeavortd_audio_codec_init(&endeavortd_audio_pdata);

	platform_add_devices(endeavortd_audio_devices,
			ARRAY_SIZE(endeavortd_audio_devices));
}

static void endeavortd_gps_init(void)
{
	int rc;
...
#endif
#if 0
static void endeavortd_modem_init(void)
{
...
#if 0
static void endeavortd_baseband_init(void)
{
        endeavortd_modem_init();

//        uhsic_phy_config.postsuspend = endeavortd_usb_hsic_postsupend;
//        uhsic_phy_config.preresume = endeavortd_usb_hsic_preresume;
//        uhsic_phy_config.usb_phy_ready = endeavortd_usb_hsic_phy_ready;
//        uhsic_phy_config.post_phy_off = endeavortd_usb_hsic_phy_off;
}
#endif
...
static struct gpio_keys_platform_data ENDEAVORTD_PROJECT_keys_platform_data = {
	.buttons	= ENDEAVORTD_PROJECT_keys,
	.nbuttons	= ARRAY_SIZE(ENDEAVORTD_PROJECT_keys),
	.wakeup_key     = enrkey_wakeup,
 };

static struct platform_device ENDEAVORTD_PROJECT_keys_device = {
	.name   = "gpio-keys",
	.id     = 0,
	.dev    = {
		.platform_data  = &ENDEAVORTD_PROJECT_keys_platform_data,
	},
};

int __init ENDEAVORTD_PROJECT_keys_init(void)
{
	int i;
	pr_info("[KEY]Enter ENDEAVORTD_PROJECT_keys_init\n");
	for (i = 0; i < ARRAY_SIZE(ENDEAVORTD_PROJECT_keys); i++)
		tegra_gpio_enable(ENDEAVORTD_PROJECT_keys[i].gpio);

	platform_device_register(&ENDEAVORTD_PROJECT_keys_device);
...
static void __init endeavortd_init(void)
{
	...
	tegra_clk_init_from_table(endeavortd_clk_init_table);
	endeavortd_pinmux_init();
	endeavortd_i2c_init();
	endeavortd_spi_init();
	endeavortd_uart_init();
	endeavortd_usb_init();
	bt_export_bd_address();
	platform_add_devices(endeavortd_devices, ARRAY_SIZE(endeavortd_devices));
	platform_device_register(&htc_headset_mgr_xe);
	board_id = htc_get_pcbid_info();
	if (machine_is_endeavortd() && (board_id >= PROJECT_PHASE_XE)) {
	...
	endeavortd_audio_init();
	//endeavortd_gps_init();
	//endeavortd_baseband_init();
	endeavor_panel_init();
	endeavortd_emc_init();
	endeavortd_sensors_init();
	endeavortd_cam_init();
...
MACHINE_START(ENDEAVORTD, "endeavortd")
	...
	.reserve        = endeavortd_reserve,
	.init_early	= tegra_init_early,
	.init_irq       = tegra_init_irq,
	.timer          = &tegra_timer,
	.init_machine   = endeavortd_init,
MACHINE_END
